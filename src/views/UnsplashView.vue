  ...
  <section id="unsplash" class="container page">
    <div class="unsplash-inner">
      ...
      <div class="unsplash-btn sch-area column">
        ...
        <SearchComp @search="searchImg" :class="'right'"/>
      </div>
...
const getImgList = async () => {
  await fetch(`https://api.unsplash.com/photos?client_id=${Access_Key.value}&per_page=30`)
...
const searchImg = async (query) => {
  if (query !== 'all') {
    await fetch(`https://api.unsplash.com/search/photos?client_id=${Access_Key.value}&per_page=30&query=${query}`)
      .then((response) => response.json())
      .then((result) => {
